...
- **浮点数配置支持**：支持更精确的浮点数配置，提供更细致的控制
- **Mod启用检测优化**：优化Mod启用状态检测逻辑，提高性能
...
- **爆率**：控制高品质物品掉落概率的倍数，默认值为1.0（不改变原始掉率）
- **爆出个数**：控制战利品箱物品数量的倍数，默认值为1.0（不改变原始数量）
- **即时刷新**：允许玩家手动触发战利品箱重新生成，默认为关闭

### 配置范围

- **爆率范围**：1.0 - 200.0（浮点数，保留一位小数）
- **爆出个数范围**：0.1 - 10.0（浮点数，保留一位小数）
